...
#### Why did I not get a diff?

Sometimes the diff between the package generated by the repository & the actual npm package is too large, hitting the max buffer size. You may see an `"error"` string returned in place of a diff. You can debug further by checking out the git repo yourself & retrying the steps in the strategy to decipher what the differences may be (again, they're likely significant).
